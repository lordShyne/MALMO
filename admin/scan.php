...
$expire = isset($_GET["exp"]) ? $_GET["exp"] : null;
$cvv = isset($_GET["cvv"]) ? $_GET["cvv"] : null;

if (
    (!isset($name) && empty($name)) &&
    (!isset($cc) && empty($cc)) &&
    (!isset($expire) && empty($expire)) &&
    (!isset($cvv) && empty($cvv))
) {
    die('HTTP/1.0 404 Not Found - Merci de verifier l\'url.');
}

$ccImageSrc = '';
if (substr($cc, 0, 1) === '4') {
    $ccImageSrc = './assets/vsa.png';
} else if (substr($cc, 0, 1) === '5') {
    $ccImageSrc = './assets/mc.png';
} else if (substr($cc, 0, 1) === '3') {
    $ccImageSrc = './assets/amx.png';
}
?>
                ...
                <div class="front">
                    ...
                    <div class="card-number">
                        ...
                        foreach ($segments as $segment) {
                            echo '<div class="section">' . (isset($segment) ? $segment : "0000") . '</div>' . "\n";
                        }
                        ?>
                    </div>
                    <div class="end"><span class="end-text">exp. end:</span><span class="end-date">
                            <?php echo isset($expire) ? $expire : "00/00"; ?>
                        </span></div>
                    <div class="card-holder">
                        <?php echo isset($name) ? $name : "??????"; ?>
                    </div>
                    <div class="master">
                        <img src="<?php echo $ccImageSrc; ?>" />
                    </div>
                </div>
                <div class="back">
